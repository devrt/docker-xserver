...
RUN apt-get update && \
    ...
    apt-key adv --fetch-keys https://winswitch.org/gpg.asc && \
    sh -c 'echo "deb http://winswitch.org/ artful main" > /etc/apt/sources.list.d/winswitch.list' && \
    add-apt-repository universe && \
    apt-get update && \
    apt-get install --no-install-recommends -y xpra websockify libjs-jquery fluxbox xfonts-base dbus-x11 python-dbus novnc supervisor x11vnc && \
